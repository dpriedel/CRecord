...
std::string_view CArrayField::UseData (std::string_view record_data, const std::vector<std::string_view>& fields_data)
{
    ...
    auto fields = field_data_ | rng::views::chunk(field_width_) | vws::transform([] (const auto& fld) { return std::string_view{fld}; });
    const auto pos = rng::find(fields, empty_field);
	if (pos != fields.end())
	{
        field_data_.resize(rng::distance(fields.begin(), pos) * field_width_);
	}
	return {field_data_} ;
